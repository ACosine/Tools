...
int Mode_Decision_for_4x4IntraBlocks (int  b8,  int  b4,  double  lambda,  int*  min_cost)
{
  ...
  double  rdcost;
  int     block_x     = 8*(b8%2)+4*(b4%2);	//++ 当前4*4块左上角像素在本宏块内的横坐标（以像素为单位）
  int     block_y     = 8*(b8/2)+4*(b4/2);	//++ 当前4*4块左上角像素在本宏块内的纵坐标（以像素为单位）
  int     pic_pix_x   = img->pix_x+block_x;	//++ 当前4*4块左上角像素在本图像内的横坐标（以像素为单位）
  int     pic_pix_y   = img->pix_y+block_y;	//++ 当前4*4块左上角像素在本图像内的纵坐标（以像素为单位）
  ...
  *min_cost = (1<<20);
  ...
  //++ 分别计算当前4*4块在9种intra4*4帧内预测模式下的预测块，参见标准：8.3.1.2 小节
  ...
  for (ipmode=0; ipmode<NO_INTRA_PMODE; ipmode++)
  {
    ...
    if( available_mode)
    {
      if (!input->rdopt)
      {
        ...
        cost += SATD (diff, input->hadamard);	//++ 计算各种预测模式（如果可用）的代价
        if (cost < *min_cost)
        {
          best_ipmode = ipmode;
          *min_cost   = cost;
  ...
  if (!input->rdopt)
  {
    ...
    for (j=0; j<4; j++)
      for (i=0; i<4; i++)
      {
        img->mpr[block_x+i][block_y+j]  = img->mprr[best_ipmode][j][i];	//++ 4*4块中每个像素的预测值
        img->m7[i][j]                   = imgY_org[pic_opix_y+j][pic_opix_x+i] - img->mprr[best_ipmode][j][i];	//++ 4*4块中每个像素的残差
      }
...
int Mode_Decision_for_8x8IntraBlocks(int b8,double lambda,int *cost)
{
  ...
  *cost = (int)floor(6.0 * lambda + 0.4999);	//++ 向下取整
...

{
  ...
  {
    if (Mode_Decision_for_8x8IntraBlocks (b8, lambda, &cost8x8))	//++ 对16*16块中的4个8*8的块循环执行
    {
      cbp |= (1<<b8);
    }
    *cost += cost8x8;
                        ...
                        double*  min_rdcost)  // <-> minimum rate-distortion cost
{
  ...
  {
	//++ 在 intra4*4 的 9 种模式中选择最佳模式并计算其代价
    currMB->cbp = Mode_Decision_for_Intra4x4Macroblock (lambda, &dummy);
  }
  else if (mode==I16MB)
  {
	//++ 在 intra16*16 的 4 种模式中选择最佳模式并计算其代价
