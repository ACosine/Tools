...
void Configure(int ac, char *av[])
{
  ...
  char *filename=DEFAULTCONFIGFILENAME;//指向文件encoder.cfg

  memset (&configinput, 0, sizeof (InputParameters));//将configinput中所有的参数替换为0，即初始化
  //Set some initial parameters.
  configinput.LevelIDC   = LEVEL_IDC;//21
  configinput.ProfileIDC = PROFILE_IDC;//88
  ...
  content = GetConfigFileContent (filename);//取文件内容
  ParseContent (content, strlen(content));//解析文件内容，将所有参数输入到input中
  printf ("\n");
  free (content);
