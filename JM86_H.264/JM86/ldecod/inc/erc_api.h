...
((currYBlockNum)%((picSizeX)>>3))	//++ 8*8块在帧中的横坐标（以8*8块为单位）

#define yPosYBlock(currYBlockNum,picSizeX) \
((currYBlockNum)/((picSizeX)>>3))	//++ 8*8块在帧中的纵坐标（以8*8块为单位）

#define xPosMB(currMBNum,picSizeX) \
((currMBNum)%((picSizeX)>>4))	//++ 宏块在帧中的横坐标（以宏块为单位）

#define yPosMB(currMBNum,picSizeX) \
((currMBNum)/((picSizeX)>>4))	//++ 宏块在帧中的纵坐标（以宏块为单位）

#define MBxy2YBlock(currXPos,currYPos,comp,picSizeX) \
((((currYPos)<<1)+((comp)>>1))*((picSizeX)>>3)+((currXPos)<<1)+((comp)&1))	//++ 如果是亮度块，计算结果则为8*8块的序号（计数方法：整帧的8*8块从左到右、从上到下）
...
typedef struct ercVariables_s
{
  ...
  int     *yCondition;		//++ 8*8的Y块的状态，取值范围参见erc_api.h文件中第51～54行
  /*  Array for conditions of U blocks */
  int     *uCondition;		//++ 8*8的U块的状态，取值范围参见erc_api.h文件中第51～54行
  /*  Array for conditions of V blocks */
  int     *vCondition;		//++ 8*8的V块的状态，取值范围参见erc_api.h文件中第51～54行
