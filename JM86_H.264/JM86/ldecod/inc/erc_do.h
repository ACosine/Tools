...
#define isSplitted(a,b) \
    ((a+((b)<<2))->regionMode >= REGMODE_SPLITTED)	//++ (a+((b)<<2)：为当前宏块的相邻宏块左上角的8*8块。如果该块的regionMode >= REGMODE_SPLITTED
													//++ 则说明该块所在的宏块被划分为了8*8的四个块。

/* this can be used as isBlock(...,INTRA) or isBlock(...,INTER_COPY) */
#define isBlock(a,b,c,d) \
    (isSplitted(a,b) ? \
     ((a+((b)<<2)+(c))->regionMode == REGMODE_##d##_8x8) : \
     ((a+((b)<<2))->regionMode == REGMODE_##d))

/* this can be used as getParam(...,mv) or getParam(...,xMin) or getParam(...,yMin) */
#define getParam(a,b,c,param) \
    (isSplitted(a,b) ? \
     ((a+((b)<<2)+(c))->param) : \
     ((a+((b)<<2))->param))
