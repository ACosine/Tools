...
typedef struct storable_picture //定义一幅图像，或者场，或者帧
{
  PictureStructure structure;//枚举类型，帧，顶场，底场
...
typedef struct frame_store     //解码图像缓冲区的帧存储结构
{
...
typedef struct decoded_picture_buffer  //解码图像缓冲区结构
{
  FrameStore  **fs;
  FrameStore  **fs_ref;
  FrameStore  **fs_ltref;
