...
int EBSPtoRBSP(byte *streamBuffer, int end_bytepos, int begin_bytepos)
{
  ...
  { //starting from begin_bytepos to avoid header information	//++ 因为NAL单元中，在开始前缀码后的第一个字节是头信息，标志该NAL单元的类型
    if(count == ZEROBYTES_SHORTSTARTCODE && streamBuffer[i] == 0x03)	//++ ZEROBYTES_SHORTSTARTCODE=2;仿校验字节中零字节的个数
    {
      i++;
      count = 0;
    }
