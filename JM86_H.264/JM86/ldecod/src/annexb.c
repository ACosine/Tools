...
int GetAnnexbNALU (NALU_t *nalu)
{
  ...
  if(Buf[pos-1]!=1)//如果不存在起始码前缀，就表明没有起始码
  {
  ...
  if(pos<3)//起始码前缀长度为3，当码流长度小于3时也不存在起始码
  {
    printf ("GetAnnexbNALU: no Start Code at the begin of the NALU, return -1\n");
    free(Buf);
    return -1;
  ...
  else if(pos==3)
  {
    ...
    LeadingZero8BitsCount = 0;//该元素结构个数
  }
  else
  {
  ...
  while (!StartCodeFound)
  {
    if (feof (bits))
    {
      ...
      while(Buf[pos-2-TrailingZero8Bits]==0)
        TrailingZero8Bits++;
      nalu->len = (pos-1)-nalu->startcodeprefix_len-LeadingZero8BitsCount-TrailingZero8Bits; //nalu的长度
      memcpy (nalu->buf, &Buf[LeadingZero8BitsCount+nalu->startcodeprefix_len], nalu->len);  //提取nalu元素，即NumBytesInNALunit  
      nalu->forbidden_bit = (nalu->buf[0]>>7) & 1;//查找流中是否存在0x000001
      nalu->nal_reference_idc = (nalu->buf[0]>>5) & 3;//查找流中是否存在0x000003
...
// printf ("GetAnnexbNALU, eof case: pos %d nalu->len %d, nalu->reference_idc %d, nal_unit_type %d \n", pos, nalu->len, nalu->nal_reference_idc, nalu->nal_unit_type);
  ...
  // have.  Hence, go back in the file如果发现另一个开始代码，则重定位输入流位置
  rewind = 0;
  if(info3 == 1)
    rewind = -4;
  else if (info2 == 1)
  ...
  {
    snprintf (errortext, ET_SIZE, "GetAnnexbNALU: Cannot fseek %d in the bit stream file", rewind);//表示失败的操作
    free(Buf);
    error(errortext, 600);
  }
 ...
 ************************************************************************
 */
static int FindStartCode (unsigned char *Buf, int zeros_in_startcode)//根据最后的标记为0x000001判断是否为开始代码
{
  int info;
  int i;
