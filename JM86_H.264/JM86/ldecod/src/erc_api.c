...
void ercReset( ercVariables_t *errorVar, int nOfMBs, int numOfSegments, int32 picSizeX )
{
  ...
  if ( errorVar && errorVar->concealment ) 
  {
    ...
    for ( i = 0; i < errorVar->nOfSegments; i++ ) 
    {
      ...
      //??? 上面三行memset语句不是已经做了同样的事情了么 memset( errorVar->yCondition, 0, 4*nOfMBs*sizeof(*errorVar->yCondition));
      for ( j = 0; j < nOfMBs; j++ ) 
      {
		...
		//++ 以上四行实际是对yCondition数组从下标0到nOfMBs*4-1置零（即以8*8的块为单位），因此可用以下四行语句代替
        //++ errorVar->yCondition[j*4] = ERC_BLOCK_EMPTY;
        //++ errorVar->yCondition[j*4+1] = ERC_BLOCK_EMPTY;
        //++ errorVar->yCondition[j*4+2] = ERC_BLOCK_EMPTY;
        //++ errorVar->yCondition[j*4+3] = ERC_BLOCK_EMPTY;
