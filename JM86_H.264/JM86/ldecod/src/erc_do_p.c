  ...
  if ( errorVar && errorVar->concealment ) 
  {
    ...
    if ( errorVar->nOfCorruptedSegments ) 
    {
      ...
      for ( columnInd = 0; columnInd < lastColumn; columnInd ++) 
      {
        ...
        {
          int par1=MBxy2YBlock(column, row, 0, picSizeX);	//++ par1为以8*8大小的块的序号
//          int par2=MBxy2YBlock(column, lastCorruptedRow, 0, picSizeX);	//++ 该行语句应该放到第115行
          if ( errorVar->yCondition[par1] <= ERC_BLOCK_CORRUPTED )	//++ 该处扫描8*8块是从上到下，并且从两头往中间
          {                           // ERC_BLOCK_CORRUPTED (1) or ERC_BLOCK_EMPTY (0)
            ...
            {
	          int par2=MBxy2YBlock(column, lastCorruptedRow, 0, picSizeX);
              /* check blocks in the current column */
              if (errorVar->yCondition[par2] > ERC_BLOCK_CORRUPTED) 
              {
                          ...
                          int32 picSizeX, int32 picSizeY, int *yCondition)
{
  int predMBNum = 0, numMBPerLine,
      ...
      minDist, currDist, i, k, bestDir,par;
  int32 regionSize;
  objectBuffer_t *currRegion;
  ...
  { /* 4 blocks loop */
    ...
    currRegion = object_list+(currMBNum<<2)+comp;	//++ 如果是亮度块，currMBNum<<2则为8*8块的序号（计数方法：第一个宏块内的4个8*8块从左到右、从上到下，第二个宏块内的8*8块从左到右、从上到下，......）
													//++ currRegion所指向的内存存放了序号为currMBNum<<2的8*8块的region mode和mv的信息

    /* set the position of the region to be concealed */
    
	par=MBNum2YBlock(currMBNum,comp,picSizeX);	//++ 如果是亮度块，则为8*8块的序号（计数方法：整帧的8*8块从左到右、从上到下）
    currRegion->xMin = (xPosYBlock(par,picSizeX)<<3);	//++ 当前8*8块左上角像素的横坐标
    currRegion->yMin = (yPosYBlock(par,picSizeX)<<3);	//++ 当前8*8块左上角像素的纵坐标
      ...
      for (i = 4; i < 8; i++) 
      {
        ...
        if (predBlocks[i] >= threshold) 
        {
          ...
          case 4:
            ...
            compSplit1 = 2;						//++ 即取上边16*16块的两个下方8*8的块
            compSplit2 = 3;						//////////////////////////////////////////
            break;
              
          case 5:
            predMBNum = currMBNum-1;			//////////////////////////////////////////
            compSplit1 = 1;						//++ 即取左边16*16块的两个右方8*8的块
            compSplit2 = 3;						//////////////////////////////////////////
            break;
              
          case 6:
            predMBNum = currMBNum+numMBPerLine;	//////////////////////////////////////////
            compSplit1 = 0;						//++ 即取下边16*16块的两个上方8*8的块
            compSplit2 = 1;						//////////////////////////////////////////
            break;
              
          case 7:
            predMBNum = currMBNum+1;			//////////////////////////////////////////
            compSplit1 = 0;						//++ 即取右边16*16块的两个左方8*8的块
            compSplit2 = 2;						//////////////////////////////////////////
            break;
          }
          ...
          else 
          {
            ...
            for (predSplitted = isSplitted(object_list, predMBNum),		//++ predSplitted：当前宏块相邻宏块左上角8*8块的分割状态
              compPred = compSplit1;
              predSplitted >= 0;
              compPred = compSplit2,
              predSplitted -= ((compSplit1 == compSplit2) ? 2 : 1)) 
            {
              ...
              if (isBlock(object_list, predMBNum, compPred, INTER_COPY))	//++ 该语句根据相邻宏块的分割状态判断该相邻宏块是否为帧间copy块
              {																//++ 如果该相邻宏块被分割为了4个8*8的块（在该语句内部调用isSplitted做判断）
																			//++ 则根据该相邻宏块第compPred个8*8的块来判断，否则根据该相邻宏块的第一个8*8块来判断
              ...
              {
                mvptr = getParam(object_list, predMBNum, compPred, mv);	//++ 获得当前相邻8*8块的运动向量，以此作为受损宏块的运动向量
...
static void buildPredRegionYUV(struct img_par *img, int32 *mv, int x, int y, byte *predMB)
{
  ...
  img->mb_x = x/MB_BLOCK_SIZE;					//++ 当前受损宏块在帧中的横坐标（以宏块为单位）
  img->mb_y = y/MB_BLOCK_SIZE;					//++ 当前受损宏块在帧中的纵坐标（以宏块为单位）
  img->block_x = img->mb_x * BLOCK_SIZE;		//++ 当前受损宏块左上角4*4亮度块在帧中的横坐标（以4*4块为单位）
  img->block_y = img->mb_y * BLOCK_SIZE;		//++ 当前受损宏块左上角4*4亮度块在帧中的纵坐标（以4*4块为单位）
  img->pix_c_x = img->mb_x * MB_BLOCK_SIZE/2;	//++ 当前受损宏块左上角4*4色度块的左上角像素在帧中的横坐标（以整像素为单位）
  img->pix_c_y = img->mb_y * MB_BLOCK_SIZE/2;	//++ 当前受损宏块左上角4*4色度块的左上角像素在帧中的纵坐标（以整像素为单位）
  ...
  // luma *******************************************************
  //++ 对错误宏块的16个4*4亮度块进行处理，处理顺序：从左到右，从上到下
  for(j=0;j<MB_BLOCK_SIZE/BLOCK_SIZE;j++)
  {
    ...
    for(i=0;i<MB_BLOCK_SIZE/BLOCK_SIZE;i++)
    {
      ...
      vec1_x = i4*4*mv_mul + mv[0];	//++ 当前4*4块左上角像素的参考像素的横坐标（运动向量以1/4像素为单位计算，所以得到的参考像素不一定为整像素）
      vec1_y = j4*4*mv_mul + mv[1];	//++ 当前4*4块左上角像素的参考像素的纵坐标（运动向量以1/4像素为单位计算，所以得到的参考像素不一定为整像素）

      get_block(ref_frame, listX[0], vec1_x,vec1_y,img,tmp_block);	//++ 6抽头滤波插值法，处理顺序：横读竖放

      for(ii=0;ii<BLOCK_SIZE;ii++)
        for(jj=0;jj<MB_BLOCK_SIZE/BLOCK_SIZE;jj++)
          img->mpr[ii+ioff][jj+joff]=tmp_block[ii][jj];	//++ img->mpr为受损宏块的预测像素值
    }
  }
  ...
  for (i = 0; i < 16; i++)
  {
    for (j = 0; j < 16; j++)
    {
	    pMB[i*16+j] = img->mpr[j][i];	//++ 将预测得出的受损宏块的像素值存入长度为256的1维指针数组
	}
  }
  pMB += 256;
  ...
  // chroma *******************************************************
  //++ 对错误宏块的4个4*4色度块进行处理
  for(uv=0;uv<2;uv++)	//++ 第一个循环处理U分量，第2个循环处理V分量
  {
    for (j=4;j<6;j++)		//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    {						//++
      joff=(j-4)*4;			//++    该双重循环控制4个4*4色度块的循环，处理顺序：从左到右，从上到下
      j4=img->pix_c_y+joff;	//++
      for(i=0;i<2;i++)		//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      {
        ioff=i*4;
        ...
        for(jj=0;jj<4;jj++)		//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        {						//++
          jf=(j4+jj)/2;			//++    该双重循环控制4*4块内16个像素值的循环，处理顺序：从左到右，从上到下
          for(ii=0;ii<4;ii++)	//++
          {						//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            if1=(i4+ii)/2;
            i1=(img->pix_c_x+ii+ioff)*f1+mv[0];	//++ 当前待隐藏像素的参考像素的横坐标（运动向量以1/4像素为单位计算，所以得到的参考像素不一定为整像素）
            j1=(img->pix_c_y+jj+joff)*f1+mv[1];	//++ 当前待隐藏像素的参考像素的纵坐标（运动向量以1/4像素为单位计算，所以得到的参考像素不一定为整像素）


            ii0=max (0, min (i1/f1, dec_picture->size_x_cr-1));	//++ 当前待隐藏像素的参考像素左上方最邻近整像素的横坐标
            jj0=max (0, min (j1/f1, dec_picture->size_y_cr-1));	//++ 当前待隐藏像素的参考像素左上方最邻近整像素的纵坐标
            ii1=max (0, min ((i1+f2)/f1, dec_picture->size_x_cr-1));	//++ 当前待隐藏像素的参考像素右下方最邻近整像素的横坐标
            jj1=max (0, min ((j1+f2)/f1, dec_picture->size_y_cr-1));	//++ 当前待隐藏像素的参考像素右下方最邻近整像素的纵坐标

            if1=(i1 & f2);	//++ 当前待隐藏像素的参考像素（不一定为整像素）与参考像素左方最邻近整像素的距离（以1/4像素为单位）
            jf1=(j1 & f2);	//++ 当前待隐藏像素的参考像素（不一定为整像素）与参考像素上方最邻近整像素的距离（以1/4像素为单位）
            if0=f1-if1;	//++ 当前待隐藏像素的参考像素（不一定为整像素）与参考像素右方最邻近整像素的距离（以1/4像素为单位）
            jf0=f1-jf1;	//++ 当前待隐藏像素的参考像素（不一定为整像素）与参考像素下方最邻近整像素的距离（以1/4像素为单位）
            img->mpr[ii+ioff][jj+joff]=(if0*jf0*listX[0][ref_frame]->imgUV[uv][jj0][ii0]+	//++ 第一项：当前待隐藏像素的参考像素左上方最邻近整像素
              if1*jf0*listX[0][ref_frame]->imgUV[uv][jj0][ii1]+								//++ 第二项：当前待隐藏像素的参考像素右上方最邻近整像素
              if0*jf1*listX[0][ref_frame]->imgUV[uv][jj1][ii0]+								//++ 第三项：当前待隐藏像素的参考像素左下方最邻近整像素
              if1*jf1*listX[0][ref_frame]->imgUV[uv][jj1][ii1]+f4)/f3;						//++ 第三项：当前待隐藏像素的参考像素右下方最邻近整像素
                        ...
                        int32 picSizeX, int32 regionSize) 
{
  ...
  for (j = ymin; j <= ymax; j++) 
  {
    for (k = xmin; k <= xmax; k++)
    {
      ...
      dec_picture->imgY[j][k] = predMB[locationTmp];	//++ 将受损宏块的预测像素值Y分量写入解码图像
    }
  }
  ...
  for (j = (ymin>>1); j <= (ymax>>1); j++) 
  {
    for (k = (xmin>>1); k <= (xmax>>1); k++)
    {
      ...
      dec_picture->imgUV[0][j][k] = predMB[locationTmp];	//++ 将受损宏块的亮度预测像素值U分量写入解码图像
      
      locationTmp += 64;
      
      dec_picture->imgUV[1][j][k] = predMB[locationTmp];	//++ 将受损宏块的亮度预测像素值V分量写入解码图像
    }
  }
}
