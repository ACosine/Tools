...
int get_mem2D(byte ***array2D, int rows, int columns)
{
  ...
  //++ 该处分配内存空间为columns*rows，而不是columns，其目的是不在后面单独对后143行每一行的
  //++ 颜色信号分配内存空间。如果该处分配内存空间为columns，即将上一行语句改为：
  //++ if(((*array2D)[0] = (byte* )calloc(columns,sizeof(byte ))) == NULL)则在下面的for循环
  //++ 中必须有对后面143行每一行单独分配内存空间的语句，即下面for循环的语句将作改动（见下面
  //++ for循环中被注释的语句）。通过检查(*array2D)[i]的地址与(*array2D)[i-1] + columns的地
  //++ 址可以证明这一点
    no_mem_exit("get_mem2D: array2D");

  for(i=1;i<rows;i++)
    (*array2D)[i] = (*array2D)[i-1] + columns ;
  //++ (*array2D)[i] = (byte* )calloc(columns,sizeof(byte));
