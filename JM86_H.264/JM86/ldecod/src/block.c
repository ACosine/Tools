...
void intrapred_chroma(struct img_par *img, int uv)
{
  ...
  int up_avail, left_avail[2], left_up_avail;	//++ left_avail[2]分别表示第0、2个4*4块左方相邻4*4块的可用性
  ...
  if (currMB->c_ipred_mode == DC_PRED_8)
  {
    for(i=0;i<4;i++)
    {
      if(up_avail)
      {
        js0=js0+imgUV[uv][up.pos_y][up.pos_x+i];	//++ 计算第0个4*4块上方4个相邻像素值之和
        js1=js1+imgUV[uv][up.pos_y][up.pos_x+i+4];	//++ 计算第1个4*4块上方4个相邻像素值之和
      }
      if(left_avail[0])
      {
        js2=js2+imgUV[uv][left[1+i].pos_y][left[1+i].pos_x];	//++ 计算第2个4*4块左方4个相邻像素值之和
	  }
      if(left_avail[1])
	  {
        js3=js3+imgUV[uv][left[1+i+4].pos_y][left[1+i+4].pos_x];	//++ 计算第3个4*4块左方4个相邻像素值之和
      }
    }
    if(up_avail && left_avail[0])
    {
      js[0][0]=(js0+js2+4)/8;	//++ 第0个4*4块采用 DC 计算的结果
      js[1][0]=(js1+2)/4;		//++ 第1个4*4块采用 DC 计算的结果
	}
    if(up_avail && left_avail[1])
	{
      js[0][1]=(js3+2)/4;		//++ 第2个4*4块采用 DC 计算的结果
      js[1][1]=(js1+js3+4)/8;	//++ 第3个4*4块采用 DC 计算的结果
    }
    if(up_avail && !left_avail[0])
    {
      js[0][0]=(js0+2)/4;
  ...
  {						//++
    ...
    for(i=0;i<2;i++)	//++++++++++++++++++++++++++++++++++++++++++
    {
      ...
      case VERT_PRED_8:
        ...
        break;
      case PLANE_8:	//++ 因为JM86只支持4:2:0采样，所以标准8.3.4.4小节中的xCF、yCF在这里均为0
 ...
 ***********************************************************************
 */
//++ 把反量化完的系数(coefficients)做逆变换（该函数实现了一维蝶形图）
//++ 4*4块的反变换顺序为：
//++ ***************
//++ * 1  2  5  6  *
//++ * 3  4  7  8  *
            ...
            int j0)              //!<
{
  ...
  for (i=0;i<BLOCK_SIZE;i++)
  {
    ...
    m6[2]=(m5[1]>>1)-m5[3];
    m6[3]=m5[1]+(m5[3]>>1);
/******** JM源代码中将最后一次一维DCT变换与预测值和变换值的求和操作连在一起做了 *********
    for (j=0;j<2;j++)
    {
      j1=3-j;
...
/********* 以下是我将上述两个操作进行了分解 **********/
  ...
  {																		//++
	  for(j=0;j<BLOCK_SIZE;j++)											//++
	  {																	//++ 分解第二步：预测值和变换值的求和
		  img->m7[i][j]=max(0,min(255,(img->m7[i][j]+					//++
			  (img->mpr[i+ioff][j+joff]<<DQ_BITS)+DQ_ROUND)>>DQ_BITS));	//++
	  }																	//++
  }																		//+++++++++++++++++++++++++++++++++++
}
