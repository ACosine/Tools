 ...
 ************************************************************************
 */
static void unmark_for_reference(FrameStore* fs)//标记未作为参考帧的帧
{

  if (fs->is_used & 1)//保留最低位，若顶场使用了，则执行顶场不作为参考帧
  {
    fs->top_field->used_for_reference = 0;
  }
  if (fs->is_used & 2)//保留次低位，若底场使用了，则执行底场不作为参考帧
  {
    fs->bottom_field->used_for_reference = 0;
  }
  if (fs->is_used == 3)//若帧使用了，则都不作为参考帧
  {
    fs->top_field->used_for_reference = 0;
    fs->bottom_field->used_for_reference = 0;
 ...
 ************************************************************************
 */
static int remove_unused_frame_from_dpb()//从DPB中去除不再需要的图像
{
  unsigned i;
 ...
 ************************************************************************
 */
void flush_dpb()//所有存储的图像都输出，清空缓冲区
{
  unsigned i;
...
//  printf("Flush remaining frames from dpb. dpb.size=%d, dpb.used_size=%d\n",dpb.size,dpb.used_size);
  ...
  for (i=0; i<dpb.used_size; i++)
  {
    unmark_for_reference (dpb.fs[i]);//标记所有未使用的帧，即将所有使用的帧标记为不作为参考帧
  }
